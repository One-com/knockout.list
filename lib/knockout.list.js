...
(function (root, factory) {
    ...
    ko.bindingHandlers.list = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            var removeIndexFromTileCache = function (index) {
                // If the focused element is removed, make sure a blur event is called beforehand, so other bindings can hook into that
                var childNodes = tileCache[index][0].childNodes;
                for (var i = 0; i < childNodes.length; i += 1) {
                    if (childNodes[i] === document.activeElement) {
                        childNodes[i].blur();
                        break;
                    }
                }
            ...
            function removeRenderTiles() {
                Object.keys(tileCache).forEach(removeIndexFromTileCache);
            }
