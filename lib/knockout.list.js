    ...
    ko.bindingHandlers.list = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            function calculateVerticalTileIndex (index) {
                // FIXME: take dividers into account
            ...
            function rowsBeforeIndex(index) {
                // FIXME: take dividers into account
                return Math.ceil(index / tilesSideBySide);
            }
            ...
            function calculateHorizontalTilePosition(index) {
                return tileWidth * calculateHorizontalTileIndex(index);
            }

            function calculateVerticalDividerPosition(index) {
                return dividersBeforeIndex(rowsBeforeIndex(index) - 1) * dividerHeight + Math.min(rowsBeforeIndex(index), dataSource.length()) * tileHeight;
            }
            ...
            function renderTile(item, index) {
                ...
                var left = calculateHorizontalTilePosition(index);

                // Make sure tile is part of the DOM before rendering to handle integration
                // with other bindings
                $tile.css({'top': top + 'px', 'left': left + 'px'}).appendTo(element);
            ...
            function calibrateHeight(callback) {
                ...
                if (tileCacheKeys.length === 0) {
                    dataSource.get(0, function (item) {
                        ...
                        tilesSideBySide = Math.floor($element.width() / tileWidth);
