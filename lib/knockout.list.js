    ...
    ko.bindingHandlers.list = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            if (bindingValue.onUpdate) {
                ...
                if (bindingValue.dataSource) {
                    dataSource = bindingValue.dataSource;
                } else {
                    var items;
            ...
            if (bindingValue.grid) {
                gridView = true; // FIXME: This should probably be set on dataSource too. Or on some ListModel
            }
            ...
            if (dataSource.scrollable) {
                var possibleScrollElements = $element.parents(dataSource.scrollable);
                if (!possibleScrollElements.length) {
                    throw new Error('Bound element does not have a parent matching the CSS selector `' + dataSource.scrollable + '`');
                } else {
